...
void loop() {
  ...
  if (packetSize) {
    ...
    if (!error) {
      ...
      float tilt  = doc["tilt"];
      float rain  = doc["rain"];
      // 1. Ambil data moist sebagai String biner terlebih dahulu
String moistBinStr = doc["moist"].as<String>(); 

// 2. Konversi String biner (basis 2) menjadi bilangan desimal (integer)
int moistInt = strtol(moistBinStr.c_str(), NULL, 2); 

// 3. Masukkan ke variabel float untuk kelembapan asli
float moist = (float)moistInt;
